...
- **Web Push 通知**: `AskUserQuestion` 等のプロアクティブ問い合わせを iOS / Android に通知。
  セッションごとに通知モード切替可
- **ターミナル入力待ち検出 + クイック返信**: 権限ダイアログ / y/n 確認 / パスワード /
  選択 picker などターミナルが入力待ちになると (サブプロセス由来も含む)、 チャット入力欄の
  直上にバナーで抜粋表示 + push 通知。 数字 / Y-n / 矢印キーのボタンでターミナル画面を
  開かずワンタップ返信
...
- **長い返答の折りたたみ**: 4,000 文字を超える返答はプレビューに畳まれ、 タップでその場に展開する。 畳んだ状態でも展開後も markdown のまま描画されるので書式が崩れない。 プレビューはコードフェンスの外の
  ブロック境界で切るため、 表やコードブロックが途中で切れることはない
- **ツール実行の inline 表示**: Bash / Edit / Write 等の全ツール呼び出しが会話内に折りたたみ行で並び、 タップで入力と結果まで展開できる。 サブエージェント実行中は現在動いてる sub-tool 名も行内表示
- **ステータスバー**: モデル / 5h・7d rate limit (リセット時刻付き) / context 使用率を常時表示。 プランモード / 残予算はセッションからデータが取れた時に表示
- **会話のフォーク**: 任意メッセージから新タブで分岐、 親 ・ 子をドロワーで階層表示
- **メッセージ履歴永続化**: lz-string で圧縮して localStorage に保存
- **マルチアカウント**: `accounts` 設定で個人 / 会社等を切り替え。 進行中の会話を ⋯ メニューから別アカウントで継続することもできる。 勤務先支給のアカウントと自分のアカウント、 のように**元々別の主体に属するアカウント**の使い分けを想定しており、 1 人が上限回避のために複数の個人契約を回す使い方は想定外 (詳細は [docs/reference/config.md](docs/reference/config.md))
...
- **拡張**: 自作の Web アプリを上部バー右端の 🧩 から開き、 チャットの上に帯で出す。 アプリは別プロセスで
...
脆弱性報告 + audit log + threat model 詳細は [SECURITY.md](SECURITY.md) 参照。
...
LaunchAgent KeepAlive で自動起動するはずなので、 まず `task status` で生死確認。 反応無ければ `task restart` で kickstart、 ログは `task logs`。 細かい復旧手順は [docs/troubleshooting/troubleshoot.md](docs/troubleshooting/troubleshoot.md)。
...
代表的なつまずきポイントと復旧手順は [docs/troubleshooting/troubleshoot.md](docs/troubleshooting/troubleshoot.md) に集約
...
- 困ったとき: [docs/troubleshooting/troubleshoot.md](docs/troubleshooting/troubleshoot.md)
