...
使い方:
    python -m backend.cli.gen_vapid [--force]

crosscut-F-21: 旧 `backend/gen_vapid.py` から `backend/cli/` 配下に移動。 ルーティング /
state を持たない ops 補助 CLI として明示する場所に整理する。 旧 path で
`python -m backend.gen_vapid` を叩いていた場合は新 path に更新が必要 (README 更新済)。
"""
import argparse
import base64
import json
