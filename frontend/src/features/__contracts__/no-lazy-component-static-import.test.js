...
//
// W2 Phase E1 (= 2026-06-29 OverlayHost 化): 4 overlay (= FilePreviewModal / FileTreePanel /
// FavoritesQuickPicker / MoonlightFrame) は AppShell.jsx の `lazy()` から features/*/index.js の
// `registerOverlay(name, { Component: () => import('./<X>.jsx'), ... })` Component spec に移行。
// 残り 3 件 (= SessionDrawer / SubagentsModal / TasksModal) は E-2 で同様に移行予定、 それまでは
// AppShell.jsx の `lazy()` に居続ける (= 移行期混在 OK)。
//
// 本 contract は両経路を gate する:
//   (a) AppShell.jsx の `lazy(() => import('../features/<x>/<X>.jsx'))` 経路 (= 旧来、 E-2 で 0 件化)
//   (b) features/<x>/index.js の `Component: () => import('./<X>.jsx')` 経路 (= 新、 E-1 で 4 件導入)
...
describe('W2 chunk-split contract: lazy 対象 component は features index.js から static import されない', () => {
  ...
  it('AppShell.jsx に lazy(() => import(...)) が 1 件以上ある (= 設計書 docs/extending.md (c) chunk 分割)', () => {
    // W2 Phase E1 時点: 3 件残置 (= SessionDrawer / SubagentsModal / TasksModal)、 E-2 で 0 件化予定。
...
describe('W2 Phase E1 chunk-split contract: overlayRegistry Component spec は同 index.js から static import されない', () => {
  ...
  it('Component spec を持つ entry が 1 件以上ある (= W2 Phase E1 で 4 件導入予定)', () => {
    // E-1 で導入される Component spec: previewPath / treeOpen / favs / desktopOpen の 4 件。
    // E-2 で 3 件追加されて合計 7 件になる予定。
