...
別のアプリ (= 自分で作った Web UI) を、 PWA の上部バー右端から開ける**拡張**として載せる手順。
...
- PWA は起動時に `GET /extensions` で一覧を取り、 各拡張の `/ext/<id>/` に `HEAD` を 1 回投げる。
  2xx が返った拡張だけ上部バーの右端にボタンが出る (= 拡張のプロセスが止まっていれば何も出ない)
...
- `icon` (任意): 上部バーのボタンに出す文字 (= 絵文字 1 つを想定)。 未指定なら 🧩
...
- 上部バー右端のボタンで開く / もう一度押すと畳む。 畳んでも拡張は動き続ける
...
| 症状 | 見るところ |
|---|---|
| ボタンが出ない | `curl -I https://<host>/ext/<id>/` が 2xx か (= 拡張のプロセス / Tailscale Serve の行)。 `curl https://<host>/extensions` に載っているか (= `config.json` の書き損じは起動ログ) |
