...
- 上部バー右端の 🧩 をタップすると、 最後に開いた拡張を開く / 畳む (= 端末ごとに覚える、 初回は一覧の先頭)。 長押し (= PC は右クリック) で一覧が開き、 別の拡張を選べる。 畳んでも拡張は動き続ける
- 開けるのは同時に 1 つ (= 別の拡張や 🖥 画面共有を開くと、 今の拡張は畳まれる)
- 帯の高さは下端の取っ手をドラッグして変えられる (= 画面の 15〜70%、 初期 30%、 端末ごとに保存)。 拡張の画面は、 高さが変わっても縦に詰まって収まる作りにしておくと使いやすい
- 帯の右上の ✕ で畳む (= 拡張は動き続ける)、 その左の ⛶ で全画面
