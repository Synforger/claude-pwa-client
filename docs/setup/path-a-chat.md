...
## バックエンド (= `task setup` で 1 発)

```bash
git clone https://github.com/Synforger/claude-pwa-client.git
cd claude-pwa-client

# task コマンドが未 install なら:
#   brew install go-task/tap/go-task    (macOS)
#   Linux / Windows: https://taskfile.dev/installation/

# 全 setup (= conda env / pip / npm / config / VAPID / git hooks)
task setup
# → backend/config.json を編集 (= agents の cwd / claude コマンドパス等、 詳細は ../reference/config.md)

# 起動
task run    # foreground、 Ctrl-C で停止 (= dev)
# or 常駐起動: 下記「backend を常駐起動する」 section
```

> `task setup` の中身を手動で踏むなら:
>
> ```bash
...
```bash
task build    # = (cd frontend && npm install && npm run build)、 dist/ をbackend が配信
```
...
```bash
task tailscale-serve    # = tailscale serve --bg http://localhost:8765
```
...
```bash
task install-service    # template を ~/Library/LaunchAgents/ にコピー + 編集案内
# → ~/Library/LaunchAgents/com.claudepwa.client.plist の絶対 path 5 箇所を編集
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.claudepwa.client.plist
task status             # LaunchAgent + port + /debug/healthcheck 12 項目を確認
```
