...
| `transport/` | backend 接続層 (= `unified.ts` / `select.ts` / `sse-*.js` / `ws-pty.js` / `ws-views.js` / `lifecycle.js` 等)。 SSE / WS の singleton 接続をここに集約。 既定は `unified.ts` の 1 本多重化接続、 consumer は `select.ts` 経由で実装を受け取る (= legacy rollback は localStorage cpc_transport=legacy) | features → transport は直接 import OK (= ADR-018)、 ports/ interface を transport が implements |
