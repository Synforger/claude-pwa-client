...
| `messages.js` | uuid 付き user / agent / system message の真値配列 (= sid 別)、 `MAX_MESSAGES_PER_SID = 200`。 tool_result 内の画像本体は載せない (= `utils/toolResult.js`、 表示は「画像」 1 語に畳むので不変)。 disk に落とす形は `useChatStorage.js::toStorableArray` が決める (= 末尾がまだ伸びている 1 件だけ除外、 それ以外は確定扱いで保存) | localStorage (lz-string 圧縮) | features/chat (= SSE handler 経由)、 useChatStorage |
