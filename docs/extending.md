...
1. **`frontend/src/overlays/<Name>.jsx`** を新規作成 (= 命名 `<Name>.jsx` + `<Name>.css`
   同居)。 共通枠の CSS は `Modal.css` を import する
2. **`frontend/src/App.jsx`** に `const <Name> = lazy(() => import('./overlays/<Name>.jsx'))`
   を 1 行追加 + `<Suspense fallback={null}>` 配下にレンダリングを追加。 初回 paint
   には乗らず、 開いた瞬間だけ chunk を fetch する設計を維持すること (= 初回 bundle 削減)
3. ESC 閉じは `frontend/src/hooks/useEscape.js`、 outside-click は
   `frontend/src/hooks/useOutsideClick.js` を使う (= 全 overlay が同じ仕組みに揃う)
4. 開閉 state は `frontend/src/hooks/useOverlays.js` に集約する (= 複数 overlay の
   排他制御 / 重ね順を 1 箇所で持つ)
...
- 常時 mount される本文の構成要素 (= ChatInput / StatusBar / ActivityBar / MessageItem 等)
  → `components/`
- 表示位置固定だが「覆いかぶさらない」 UI (= MoonlightFrame の iframe) → `components/`
- 純データ helper / hook → `utils/` / `hooks/`

詳細規約は `frontend/src/overlays/_README.md`。
