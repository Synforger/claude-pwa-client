...
4. **自動起動は systemd user service で** (`REDACTED_PATH/systemd/user/claudepwa.service`):
   ```ini
   [Unit]
...
`REDACTED_PATH/sunshine/sunshine.conf` に:
...
`REDACTED_PATH/sunshine-audio-switch/switch.sh` (= 抜粋):
